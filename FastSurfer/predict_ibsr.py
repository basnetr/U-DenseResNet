...
def options_parse():
    ...
    parser.add_argument('--in_dir', '--input_dir', dest='idir', help='name of directory to process. Default: IBSR_nifti_stripped_conformed',
                        default='IBSR_nifti_stripped_conformed')
    parser.add_argument('--out_dir', '--output_dir', dest='odir', default='IBSR_nifti_stripped_prediction',
                        help='name of directory to output prediction. Default: IBSR_nifti_stripped_prediction')
...
if __name__ == "__main__":
    ...
    if options.simple_run:
        ...
        if not op.exists(options.odir):
            makedirs(options.odir)

        niigzimgfiles = glob.glob(op.join(options.idir, 'IBSR_*_img_conformed.nii.gz'))

        for niigziimgfile in niigzimgfiles:

        	out_niigziimgfile = op.join( options.odir, path_leaf(niigziimgfile).replace('_img_conformed.nii.gz', '_prd_conformed.mgz') )
        	fastsurfercnn(niigziimgfile, out_niigziimgfile, use_cuda, small_gpu, logger, options)


# python predict_ibsr.py --in_dir "/home/ramesh/fs/FastSurfer/Tutorial/data/IBSR_nifti_stripped_conformed" \
# --out_dir /home/ramesh/fs/FastSurfer/Tutorial/output/IBSR_nifti_stripped_prediction \
# --order 1 \
# --network_sagittal_path /home/ramesh/fs/FastSurfer/checkpoints/Sagittal_Weights_FastSurferCNN/ckpts/Epoch_30_training_state.pkl \
# --network_axial_path /home/ramesh/fs/FastSurfer/checkpoints/Axial_Weights_FastSurferCNN/ckpts/Epoch_30_training_state.pkl \
# --network_coronal_path /home/ramesh/fs/FastSurfer/checkpoints/Coronal_Weights_FastSurferCNN/ckpts/Epoch_30_training_state.pkl \
# --batch_size 8 \
# --simple_run \
# --run_viewagg_on check